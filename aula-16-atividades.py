"""
Atividade 001 - Operadores aritméticos

A. Faça um programa que peça 3 valores , depois calcule e imprima  a soma e a multiplicação desses valores. 
B. Faça um programa que peça o ano do seu nascimento e calcule quantos anos você faz esse ano.
C. Faça um programa que peça 4 notas, após a entrada de dados calcular a média das notas digitadas.
D#. Faça um programa que receba e divida 2 números. A saída da divisão precisará ser formatada com 4 casas decimais.
E. Faça um programa que receba um número inteiro e mostre o sucessor e antecessor.
H. Faça um programa que receba um número qualquer e calcule o dobro e o triplo desse número.
I. Faça um programa que converta metros em centímetros e milímetros.
J. Faça um programa que receba um número inteiro, depois imprima sua tabuada de multiplicação.
K. Faça um programa que receba um valor em reais, depois calcule quantos dólares daria para comprar com esse valor.
L#. Faça um programa com entrada de dados para calcular o perímetro de um retângulo.

Observação
Faça um arquivo '.py' para cada atividade. 
É obrigatório a saída formatada!
Variáveis bem definidas
O código precisa ter o cabeçalho padrão → Enunciado, nome e data. 
o código precisa ser comentado
Enviar somente os exercícios que possuem '#'

"""

